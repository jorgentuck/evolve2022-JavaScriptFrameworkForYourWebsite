...
$(document).ready(function () {
  ...
  const populateData = () => {
    ...
    $.getJSON(feed, (data) => {
      ...
      paginateData(1);
      //  add filter select and search bar
      addFilters();
    });
  };
...
//  update items shown to match department dropdown and search bar terms
function updateData(term, dept) {
  //  empty the displaying div first
  listingDiv.empty();
  //  clear out the newsData array
  newsData.length = 0;
  //  check for any search terms
  if (term.length > 0) {
    //  check to see if department dropdown has been used
    if (dept == 'Department') {
      //  iterate over the initial data from the ajax call
      $.each(initData, (k, v) => {
        //  check to see if the supplied search term exists in at least one of the following locations: image_caption, image_alt, summary, author, department, title, item_date and then create cards for matching items
        if (v.image_caption.toLowerCase().indexOf(term) > -1 || v.image_alt.toLowerCase().indexOf(term) > -1 || v.summary.toLowerCase().indexOf(term) > -1 || v.author.toLowerCase().indexOf(term) > -1 || v.department.toLowerCase().indexOf(term) > -1 || v.title.toLowerCase().indexOf(term) > -1 || v.item_date.toLowerCase().indexOf(term) > -1) {
          //  create news card
          cardCreation(v);
        }
      });
      //  Department dropdown was used
    } else {
      //  iterate over the initial data from the ajax call
      $.each(initData, (k, v) => {
        //  check to see if the department matches AND the supplied search term exists in at least one of the following locations: image_caption, image_alt, summary, author, department, title, item_date and then create cards for matching items
        if (v.department.toLowerCase() == dept && (v.image_caption.toLowerCase().indexOf(term) > -1 || v.image_alt.toLowerCase().indexOf(term) > -1 || v.summary.toLowerCase().indexOf(term) > -1 || v.author.toLowerCase().indexOf(term) > -1 || v.title.toLowerCase().indexOf(term) > -1 || v.item_date.toLowerCase().indexOf(term) > -1)) {
          //  create news card
          cardCreation(v);
        }
      });
    ...
    //  check to see if department dropdown has been used
    if (dept == 'Department') {
      //  no department was selected and no search terms so display all items
      $.each(initData, (k, v) => {
        //  create news card
        cardCreation(v);
      });
    } else {
      //  department was selected, but no search terms
      $.each(initData, (k, v) => {
        //  check if department matches
        if (v.department.toLowerCase() == dept) {
          //  create news card
          cardCreation(v);
        }
      });
...
//  add department select and search bar
function addFilters() {
  //  clear any filters
  listingFilter.empty();
  //  create appendData string to hold our new html
  let appendData = new String();
  //  add html for container, row, and the start of the select
  appendData = appendData.concat(`<div class="container-fluid"><div class="row my-3 float-right"><select id="departmentSelect" class="form-select" aria-label="Department Select">
 <option selected>Department</option>`);
  //  add each select option based on the departments used in the file
  $.each(deptArr, (k, v) => {
    appendData = appendData.concat('<option value="' + v.toLowerCase() + '">' + v + '</option>');
  });
  //  end html for select
  appendData = appendData.concat(`</select>`);
  //  html for the search bar, and closing html for the filter section
  appendData = appendData.concat(`<form id="searchForm" class="form-inline mx-1">
      <input id="searchFormInput" class="form-control" type="search" placeholder="Search" aria-label="Search">
    </form></div></div>`);
  //  add the html to the filter area
  listingFilter.append(appendData);
}

//  on submit stop page from refreshing
$(document).on('submit', '#searchForm', (e) => {
  e.preventDefault();
});
//  on keyup in search bar stop page from refreshing, and update listing
$(document).on('keyup', '#searchFormInput', (e) => {
  e.preventDefault();
  let searchTerm = $(e.target).val();
  let searchDept = $('#departmentSelect').val();
  updateData(searchTerm, searchDept);
});
//  clearing search bar text stop page from refreshing, and update listing
$(document).on('search', '#searchFormInput', (e) => {
  e.preventDefault();
  let searchTerm = $(e.target).val();
  let searchDept = $('#departmentSelect').val();
  updateData(searchTerm, searchDept);
});
//  when department changes, stop from refresing, and update listing
$(document).on('change', '#departmentSelect', (e) => {
  e.preventDefault();
  let searchTerm = $('#searchFormInput').val();
  let searchDept = $(e.target).val();
  updateData(searchTerm, searchDept);
});
