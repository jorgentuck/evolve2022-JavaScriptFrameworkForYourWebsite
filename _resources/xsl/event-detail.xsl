...
<xsl:stylesheet version="3.0"
	...
	<xsl:template name="page-content">
		...
		<xsl:variable name="location" select="ou:multiedit-field('location')"/>
		<xsl:variable name="display_datetime">
			<xsl:choose>
				...
				<xsl:when test="
					...
					<xsl:value-of select="$item_date =>  ou:to-dateTime() => format-dateTime('[MNn,3-3] [D], [Y0001] [h1]:[m01] [P]')"/>
				</xsl:when>
				<xsl:otherwise>
			...
			<div class="container">
				...
				<div class="row"> 
					<div class="col-12 mb-3">
						...
						<h2 class="title-decorative font-size-sm">{$display_datetime}<xsl:if test="$location !=''"> | {$location}</xsl:if></h2>
					</div>
				</div>


				<xsl:if test="$heading-image !=''">
					<div class="row">  
						<div class="col-xl-10">
							<div class="figure">
								<xsl:choose>
	...
	<xsl:template name="related-news">
		...
		<xsl:variable name="department">
			<xsl:if test="ouc:pcf-param('department') !=''">
				<xsl:if test="$tags !=''">and</xsl:if>
				department[translate(text(),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')='{ouc:pcf-param('department') => normalize-space() => lower-case()}']</xsl:if>
		</xsl:variable>
				...
				<xpath>
					<xsl:choose>
						<xsl:when test="$tags !='' or ouc:pcf-param('department') !=''">
							items/item[@href!='{$ou:path}'and{$filters}{$department}]
						</xsl:when>
						<xsl:otherwise>
