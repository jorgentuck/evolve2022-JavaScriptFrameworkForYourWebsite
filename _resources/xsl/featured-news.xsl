...
<xsl:stylesheet version="3.0"
	...
	<xsl:template name="page-content">
		...
		<xsl:param name="events-link-text" select="ou:pcf-param('events-link-text') => normalize-space()" />
		<xsl:param name="events-link" select="ou:pcf-param('events-link') => normalize-space()" />
		...
		<div class="content" id="main-content">
				...
				<div class="container">
					...
					<div class="row">
						<div class="col-lg-5 col-xl-6">
							<xsl:if test="ouc:pcf-param('featured-1') !=''">
								<xsl:call-template name="featured-news">
									<xsl:with-param name="url" select="ouc:pcf-param('featured-1')"></xsl:with-param>
									<xsl:with-param name="position">1</xsl:with-param>
								</xsl:call-template>
							</xsl:if>
						</div>
						<div class="col-lg-7 col-xl-6 pl-xl-5">
							<xsl:if test="ouc:pcf-param('featured-2') !=''">
								<xsl:call-template name="featured-news">
									<xsl:with-param name="url" select="ouc:pcf-param('featured-2')"></xsl:with-param>
									<xsl:with-param name="position">2</xsl:with-param>
								</xsl:call-template>
							</xsl:if>
							<xsl:if test="ouc:pcf-param('featured-3') !=''">
								<xsl:call-template name="featured-news">
									<xsl:with-param name="url" select="ouc:pcf-param('featured-3')"></xsl:with-param>
			...
			<div class="container">
				<div class="row mt-5">
					...
					<div class="col-md-6 col-lg-4 offset-lg-1 events">
						<div class="news-events-header">
							<h2>Events</h2> 
							<xsl:if test="$events-link-text !='' and $events-link !=''">
								<a href="{$events-link}" class="events"><span class="far fa-calendar-alt"></span> {$events-link-text}</a>
							</xsl:if>
						</div>
	...
	<xsl:template name="featured-news">
		<xsl:param name="url"/>
		<xsl:param name="position"/>
		<xsl:variable name="url-adjusted">
			<xsl:choose>
				<xsl:when test="contains($url, 'html')">
					<xsl:value-of select="substring-before($url,'html')"></xsl:value-of>    
				</xsl:when>
				<xsl:when test="contains($url, 'inc')">
					<xsl:value-of select="substring-before($url,'inc')"></xsl:value-of>    
				</xsl:when>
				<xsl:when test="contains($url, 'php')">
					<xsl:value-of select="substring-before($url,'php')"></xsl:value-of>    
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="substring-before($url,'xml')"></xsl:value-of>    
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:call-template name="dmc">
			<xsl:with-param name="options">
				...
				<xpath>items/item[@href='{$url-adjusted}html']</xpath>
	...
	<xsl:template name="news-list">
		<xsl:param name="items-per-page" select="(ou:pcf-param('items-per-page') => number(), 4)[boolean(.)][1]"/>
...
<xsl:apply-templates select="$filter-predicate"/>
	...
	<xsl:template name="events-list">
		<xsl:param name="items-per-page" select="(ou:pcf-param('events-items-per-page') => number(), 4)[boolean(.)][1]"/>
		<xsl:param name="filter-strength" select="ouc:pcf-param('events-filter-strength')"/>

		<!-- New Way with predicate function that uses one parameter -->
		<xsl:variable name="logical-operator" select="
													  if ($filter-strength = 'and') then
													  true()
													  else
													  false()"/>

		<xsl:variable name="filter-predicate" select="ou:get-filter-predicate(ou:pcf-param('events-filter-tags'), 'tags/tag', $logical-operator, false())"/>
...
false()"/>
<xsl:variable name="filter-predicate" select="ou:get-filter-predicate-multi(ou:pcf-param('events-filter-tags'), 'tags/tag', $logical-operator, false())"/>
<xsl:variable name="dept-predicate" select="ou:get-filter-predicate-multi(ou:pcf-param('department'), 'department', false(), false())"/>
<xsl:variable name="year-predicate" select="ou:get-filter-predicate-multi(ou:pcf-param('year-filter'), 'year/year', false(), false())"/> -->
